...
const getHeroTreeAsync = async function(email: string) {
  ...
  // hero.accountRep = await getAccountRepAsync(hero.id);
  // or
  ...
  // await getShippingStatusAsync(orders???)
  // create an array of calls we can make, not actually firing off async calls
